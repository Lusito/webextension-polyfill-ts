...
function getReducedRefJson(json: any, attr: string) {
    ...
    id = id
        .replace("contextMenusInternal", "menusInternal")
        .replace("UnrecognizedProperty", "manifest.UnrecognizedProperty");
...
function validateSchemaProperty(json: any) {
    ...
    if (json.choices) SchemaChoicesProperty.validate(json);
    else if (json.value) SchemaValueProperty.validate(json);
    else {
        switch (json.type) {
            case "number":
            case "integer":
